...
logger_t lg2 = {-1, LOG_LEVEL_VERB, NULL};
logger_t lg3 = {-1, LOG_LEVEL_VERB, NULL};
...
#ifdef _LOGGER_PID_
    len = snprintf(buffer, sizeof(buffer), "[%04d-%02d-%02d %02d:%02d:%02d.%06lu][%d]|"
            "%s|%s:%d:(%s):", local.tm_year+1900, local.tm_mon+1, local.tm_mday, 
            local.tm_hour, local.tm_min, local.tm_sec, tv.tv_usec, getpid(), 
